...
def derivative_filter():
    ...
    filtered_1st = cv2.filter2D(org_img, cv2.CV_32FC1, deriv_1st_kernel.transpose())

    # Apply Laplacian Filter
    filtered_2nd = cv2.filter2D(org_img, cv2.CV_32FC1, deriv_2nd_kernel.transpose())
...
def derivative_filter_with_gaussian(band, init_sigma, k, normalize=False):
    ...
    for i in range(sigma_num):
        ...
        # Generate Images.
        filtered_images.append(cv2.filter2D(org_img, -1, gauss_kernels[-1].transpose()))
        filtered_1st_images.append(cv2.filter2D(filtered_images[-1], cv2.CV_32FC1, deriv_1st_kernel.transpose()) * (sigma if normalize else 1.0))
        filtered_2nd_images.append(cv2.filter2D(filtered_images[-1], cv2.CV_32FC1, deriv_2nd_kernel.transpose()) * (sigma**2 if normalize else 1.0))
...
def filter_sample_image():
    ...
    # Genearate 1st Deriv Kernel    
    deriv_1st_kernel_x = np.array([[-1/2, 0.0, 1/2]], dtype=np.float)
    gray_img_1st_x = cv2.filter2D(gray_img, cv2.CV_32FC1, deriv_1st_kernel_x) + 127
    gray_img_1st_x = np.uint8(gray_img_1st_x)

    deriv_1st_kernel_y = np.array([[-1/2], [0.0], [1/2]], dtype=np.float)
    gray_img_1st_y = cv2.filter2D(gray_img, cv2.CV_32FC1, deriv_1st_kernel_y) + 127
    gray_img_1st_y = np.uint8(gray_img_1st_y)

    # Genearate 2nd Deriv Kernel
    deriv_2nd_kernel = np.array([[0.0, 1.0, 0.0], [1.0, -4.0, 1.0], [0.0, 1.0, 0.0]],dtype=np.float)
    gray_img_2nd = cv2.filter2D(gray_img, cv2.CV_32FC1, deriv_2nd_kernel) + 127
...
if __name__ == "__main__":
    ...
    #derivative_filter_with_gaussian(band=51, init_sigma=0.5, k=np.sqrt(2), normalize=False)    

    #derivative_filter_with_gaussian(band=51, init_sigma=0.5, k=np.sqrt(2), normalize=True)
